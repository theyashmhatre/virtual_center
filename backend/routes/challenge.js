...
router.post("/create-challenge", [passport.authenticate("jwt", { session: false }), upload.single('cover_image')], (req, res) => {
    try {
        const { errors, isValid } = createChallengeValidation(req);

        if (!isValid) return res.status(400).json(errors);

        const { challengeTitle, challengeDescription, userId, endDate, tags, supportingMedia, reward } = req.body;
        ...
        mysqlConnection.beginTransaction(function (err) {
            ...
            mysqlConnection.query(`INSERT IGNORE INTO tag (name) VALUES ?`, [tagValues], (sqlErr, result, fields) => {
                if (sqlErr) {
                ...
                mysqlConnection.query(`INSERT INTO challenge SET ?`, newChallenge, (sqlErr, result, fields) => {
                    ...
                    mysqlConnection.query(`INSERT INTO tag_map (challenge_id, tag_id) SELECT ${newChallengeId}, t.tag_id from tag t WHERE t.name IN ?`, [[tagValues]], (sqlErr, result, fields) => {
                        if (sqlErr) {
                            return mysqlConnection.rollback(function () {
                                throw sqlErr;
                            });
...
router.post("/edit-challenge/:challengeId", passport.authenticate("jwt", { session: false }), (req, res) => {
    try {
        let { challengeId } = req.params;

        if (!challengeId) return res.status(400).json({ main: "Invalid Request", devMsg: "No challenge id found" });

        const { errors, isValid } = editChallengeValidation(req);

        if (!isValid) return res.status(400).json(errors);

        const { challengeTitle, challengeDescription, userId, endDate, tags, supportingMedia, reward, status } = req.body;
        ...
        mysqlConnection.query(`SELECT * from challenge where challenge_id = ${challengeId}`, (sqlErr, result, fields) => {
            if (sqlErr) {
                return res.status(500).json({
                    main: "Something went wrong. Please try again.",
                    devError: sqlErr,
                ...
                mysqlConnection.query(`UPDATE challenge SET ? WHERE challenge_id = ?`, [updatedChallenge, challengeId], (sqlErr, result, fields) => {
                    if (sqlErr) {
                        console.log(sqlErr);
                        return res.status(500).json({
                            main: "Something went wrong. Please try again.",
                        ...
                        res.status(200).json({ main: "Challenge updated Successfully." });
                    }
                });
            }
        });
...
router.get("/get-single-challenge/:challengeId", passport.authenticate("jwt", { session: false }), (req, res) => {
    try {
        let { challengeId } = req.params;
...
router.get("/get-challenges/:pageNum", passport.authenticate("jwt", { session: false }), (req, res) => {
    try {
        ...
        mysqlConnection.query(`SELECT * from challenge LIMIT ? OFFSET ?`, [limit, offset], (sqlErr, result, fields) => {
...
router.get("/get-challenges-using-tags/:tagsArray", passport.authenticate("jwt", { session: false }), (req, res) => {
    try {
        let { tagsArray } = req.params;

        tagsArray = JSON.parse(tagsArray);

        if (!tagsArray) return res.status(400).json({ main: "Invalid Request", devMsg: "No tag found in request" });
...
router.get("/search/:queryString/:pageNum", passport.authenticate("jwt", { session: false }), (req, res) => {
    try {
        ...
        if (!queryString) return res.status(400).json({ main: "Invalid Request", devMsg: "No query string id found" });
        if (!pageNum) return res.status(400).json({ main: "Invalid Request", devMsg: "No page number found in the request" });
            ...
            (sqlErr, result, fields) => {
                ...
                if (sqlErr) {
                    ...
                    return res.status(500).json({
                        main: "Something went wrong. Please try again.",
                        devError: sqlErr,
                        devMsg: "Error occured while searching challenges using a query string",
                    });
...
router.get("/search/all/:queryString/:pageNum", passport.authenticate("jwt", { session: false }), (req, res) => {
    try {
        const { queryString, pageNum } = req.params;
...
router.delete("/delete-challenge/:challengeId", passport.authenticate("jwt", { session: false }), (req, res) => {
    try {
        ...
        mysqlConnection.query(`SELECT * from challenge where challenge_id = ${challengeId}`, (sqlErr, result, fields) => {
            if (sqlErr) {
                ...
                res.status(401).json({ main: "You are not allowed to perform this operation.", devMsg: "Request Unauthorised. Mismatch of user ID and challenge creator ID" });

            } else {
                mysqlConnection.query(`DELETE from challenge WHERE challenge_id = ${challengeId}`, (sqlErr, result, fields) => {
                    if (sqlErr) {
                        console.log(sqlErr);
                        return res.status(500).json({
                            main: "Something went wrong. Please try again.",
                        ...
                        res.status(200).json({ main: "Challenge deleted successfully." });
                    }
                });
            }
        });
